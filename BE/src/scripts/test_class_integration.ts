...
async function runTest() {
  ...
  try {
    const res = await fetch(`${BASE_URL}/classes/bulk`, {
      method: "POST",
    ...
    if (res.status !== 200) throw new Error("Failed to create classes");
  } catch (error) {
  ...
  try {
    const res = await fetch(`${BASE_URL}/classes/timetable/import`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (res.status !== 200) throw new Error("Failed to import timetable");
  } catch (error) {
    console.error("❌ Failed:", error);
    return;
  }
  ...
  try {
    ...
    const res = await fetch(`${BASE_URL}/classes/${encodeURIComponent(className)}/timetable`);
    const data: any = await res.json();
